...
panel(s, 4.90, 1.30, CW - 4.35, 2.55, [
    ...
    ("3.  標準 MPI API で書いた GEMM ベンチを、1 行も変えずに実機で PASS させた", {"space": 4}),
    ("     (OpenMP は API 互換のスタブ。ノード内 2 コア並列はまだ)", {"space": 0, "size": 12,
                                                                     "color": MUTED}),
], size=15, head="3 行サマリ")
...
tiles = [
    ...
    ("max_payload 512 B", "ヘッダは 300、バイナリは 672、実測は 512"),
    ("1,788 checks", "実機に焼く前のホストテスト"),
    ("5 連タップ", "耳を叩くと両バッズで再実行"),
    ("80+ commits / 2 週間", "2026-08-18 → 09-01"),
]
...
text(s, 5.72, 2.46, 1.88, 0.62, "左右間の\nTWS 制御チャネル\n(cmdcode 0x8201 を自作)",
     size=10, color=MUTED, align=PP_ALIGN.CENTER, pad=0.02, space=0)
text(s, M, 3.10, CW, 0.26,
     "※ rank は左右ストラップ由来で固定。IBRT のロール (master / slave) は起動ごとに入れ替わる",
     size=10, color=MUTED, align=PP_ALIGN.CENTER, pad=0.0)
rect(s, 1.05, 3.40, 11.22, 0.44, FILL, BOX)
text(s, 1.05, 3.40, 11.22, 0.44,
     "Bluetooth SPP (自作ログチャネル)  =  Bluetooth 上のシリアルポート。UART とは独立の経路",
     size=13, bold=True, anchor=MSO_ANCHOR.MIDDLE, align=PP_ALIGN.CENTER, pad=0.05)
arrow(s, 6.44, 3.94, 0.44, 0.34, MSO_SHAPE.DOWN_ARROW)
...
plain_table(s, M, 1.24, 7.35, 4.6, [
    ...
    ["ファーム", "OpenPineBuds (pine64) — アプリ層は OSS (BT 中核は .a)"],
    ["ビルド", "Docker + Make、gnu++98 / -fno-exceptions / -fno-rtti"],
    ["デバッグ", "充電ケースが USB → デュアル UART を兼ねる (2 Mbaud)"],
    ["実測残量", "RAM 約 330 KB / Flash 約 3.2 MB (2026-08-30 の .map)"],
...
text(s, M, 1.22, CW, 0.52,
     "当初は手持ちの TWS (左右が独立した完全ワイヤレス) でやりたかった。 →  自作コードを載せる手段が見つからなかった。",
...
text(s, M, 5.05, CW, 0.95, [
    ...
    ("・ 分解して SWD を引き出す道もあるが、左右 2 個を同じ状態に保って何十回も焼き直す用途には向かない", {"space": 5}),
    ("・ これは手元の機種と自分が調べられた範囲の話で、すべての TWS がそうだと主張するものではない", {"space": 0,
                                                                                                  "size": 11.5}),
...
layers = [
    ...
    ("adapters/mpi/  +  adapters/omp/", "自作 MPI サブセット (Init/Send/Recv/Barrier/Allreduce/Isend/Irecv/Wait) / OpenMP は API 互換スタブ", FILL),
...
panel(s, 7.10, 4.62, 5.68, 2.20, [
    ("・ 256 B 超は 268 B フレームに断片化 (ペイロード 256 B)", {"space": 5}),
    ("   クレジット制御 W=2 で in-flight を 536 B に抑える", {"space": 8, "color": MUTED, "size": 11}),
    ("・ 実測 512 B はメッセージ上限。ヘッダの 300 でも", {"space": 3}),
    ("   blob の 672 でもなかった", {"space": 8, "color": MUTED, "size": 11}),
...
code(s, 8.35, 1.22, 4.43, 2.30, [
    "[mpi] finalize done rank=0",
    "### EXCEPTION ###",
    "CFSR =00000082   ; MMARVALID|DACCVIOL",
    ("MMFAR=00000034", {"bold": True}),
    "FaultCause: (Data access violation)",
    ("Current Task    : 0   <- 走るタスクが居ない", {"bold": True}),
    "New Running Task: 255  ; idle TCB",
], size=10.5, head="Run 2 : 両バッズ・毎ブート・17.5 秒周期")
panel(s, 8.35, 3.68, 4.43, 1.20, [
    ("0x34 = RTX の OS_TCB 構造体の swap_out_time オフセット。", {"space": 4}),
    ("終了済みスレッドの TCB を NULL 経由で触っていた。", {"space": 0, "bold": True}),
], size=11, head="MMFAR が犯人を名指しした")
panel(s, 8.35, 5.04, 4.43, 1.20, [
    ("純ロジックは全部ホストのテストを先に書いた。実機で出た", {"space": 4}),
    ("SPP のバグも 5 本テストに落としてから直した。", {"space": 0}),
], size=11, head="効いた運用")
...
panel(s, M, 1.84, 6.05, 2.62, [
    ...
    ("   Bluetooth SPP で Windows に流す", {"space": 7, "color": MUTED}),
    ("・ SPP = Bluetooth 上のシリアルポート。PC 側は", {"space": 3}),
    ("   spp_tail.py COM6 で受けるだけ", {"space": 7, "color": MUTED}),
    ("・ #<seq> 番号で欠落・重複を機械検出できる", {"space": 3}),
    ("・ UART とは独立の経路なので、原理的に USB は要らない", {"space": 3, "bold": True}),
    ("   (実測はすべて USB 接続下。抜いて動かしたランはまだ無い)", {"space": 0, "color": MUTED,
                                                                    "size": 11}),
...
steps = [("① 5 連タップ", "叩かれた側だけが\nキーイベントを受け取る"),
         ("② 相手に START を送る", "左右間の制御チャネルに\n5 バイトのフレームを 1 本"),
         ("③ 相手も起きる", "受信側は待機セマフォを\n解放して計算に入る"),
         ("④ 両バッズで GEMM", "rank 0 が checksum を検証\n[mpi] run #1 done rank=0")]
sw = 2.72
gap = (CW - 4 * sw) / 3
for i, (h, b) in enumerate(steps):
    ...
    text(s, sx, FY + 0.46, sw, 0.86, b, size=10.5, color=MUTED,
...
checks = [
    ...
    "EXCEPTION 0 回・MPI フレーム送信失敗 0 回・リブート無し",
]
rect(s, 8.32, 1.22, 4.46, 2.90, FILL, BOX)
rect(s, 8.32, 1.22, 4.46, 0.42, FILL2, BOX)
text(s, 8.32, 1.22, 4.46, 0.42, "合否条件 — すべて充足", size=13.5, bold=True,
     anchor=MSO_ANCHOR.MIDDLE, pad=0.12)
for i, c in enumerate(checks):
    cy = 1.80 + i * 0.58
    text(s, 8.44, cy, 0.30, 0.42, "✓", size=16, bold=True, color=OK, pad=0.0)
    text(s, 8.80, cy - 0.02, 3.90, 0.54, c, size=11.5, pad=0.0)
panel(s, 8.32, 4.28, 4.46, 2.44, [
...
panel(s, 7.30, 4.14, 5.48, 2.58, [
    ("・ タップした側が先に走り出し、相手は START の伝搬待ちで合流する", {"space": 4}),
    ("   → タップした側 (rank 0) の elapsed が伸びる  (#1 / #3)", {"space": 8, "color": MUTED}),
    ("・ このランで COM6 に出たのは左 (rank 1) のログだけ", {"space": 4}),
    ("   IBRT の slave 側の SPP 送信は電波に乗らないので、rank 0 の", {"space": 3, "color": MUTED}),
    ("   GEMM-MPI 行は UART 側にしか出ていない (未解決)", {"space": 0, "color": MUTED}),
], size=11.5, head="読み方と、正直な注記")
code(s, M, 3.96, 6.55, 2.00, [
    "app_key_compute_run event 12",
    ("[mpi] run #1 trigger=local peer_notified=1", {"bold": True}),
    "GEMM-MPI N=32 rank=0 size=2 checksum=32768.000000",
    "         expect=32768.000000 PASS",
...
plain_table(s, M, 1.22, 7.55, 1.80, [
    ...
    ["GEMM N=32  2 ノード MPI", ("12 〜 13 ms", {"bold": True}),
     ("通信込み。並列化して遅くなった", {"bold": True})],
    ["左右間 実効ペイロード", "512 B", "ヘッダの 300 でも blob の 672 でもない"],
    ["アイドルからの往復 RTT", "最大 407 ms", "ランごとに変動 (avg 93 〜 278 ms)"],
], widths=[2.6, 2.0, 3.4], size=10.5, row_h=0.36)
rect(s, M, 3.16, 7.55, 1.00, FILL2, BOX)
text(s, M, 3.26, 7.55, 0.46, "単コア 5〜10 ms   →   2 ノード 12〜13 ms", size=18,
     bold=True, align=PP_ALIGN.CENTER, pad=0.05)
text(s, M, 3.74, 7.55, 0.32, "分散したぶんだけ遅くなった", size=12.5, align=PP_ALIGN.CENTER, pad=0.05)
panel(s, M, 4.32, 7.55, 1.68, [
    ("・ 分割で浮く計算は 数 ms (単コア 5〜10 ms の半分)。リンクが張れている間の往復も 数 ms。",
     {"space": 5}),
    ("  差し引きで通信のほうが勝つ = N=32 では分散する意味がない", {"space": 9}),
    ("・ 407 ms の RTT は 100 回連続プローブ中の値で、GEMM 実行中のレイテンシではない。",
     {"space": 5, "color": MUTED}),
    ("  省電力 (sniff) に落ちた状態から叩き起こすとここまで伸びる (タップ再実行の 128 ms)",
     {"space": 0, "color": MUTED}),
], size=11.5, head="12 〜 13 ms の読み方")
rect(s, 8.32, 1.22, 4.46, 4.78, FILL, BOX)
rect(s, 8.32, 1.22, 4.46, 0.42, FILL2, BOX)
text(s, 8.32, 1.22, 4.46, 0.42, "なぜ遅いのか  =  この実験の本題", size=13.5, bold=True,
     anchor=MSO_ANCHOR.MIDDLE, pad=0.12)
rows13 = [("① 分割で浮く計算", "2 × 32³ ≒ 65,000 flop を半分に\n→  数 ms"),
          ("② 往復に要る時間", "リンクが生きていれば 数 ms\n→  ① とほぼ相殺、むしろ負け"),
          ("③ アイドルからの往復", "最大 407 ms\n=  単コア GEMM 40 〜 80 回ぶん")]
for i, (h, b) in enumerate(rows13):
    ry = 1.78 + i * 1.02
    rect(s, 8.52, ry, 4.06, 0.92, WHITE, BOX)
    text(s, 8.52, ry + 0.04, 4.06, 0.32, h, size=12, bold=True, align=PP_ALIGN.CENTER, pad=0.04)
    text(s, 8.52, ry + 0.34, 4.06, 0.56, b, size=11, color=MUTED, align=PP_ALIGN.CENTER,
         pad=0.04, space=1)
text(s, 8.44, 4.94, 4.22, 0.96,
     "「計算量 / 通信量」が小さい問題を\n分散してはいけない", size=15, bold=True,
     align=PP_ALIGN.CENTER, pad=0.06, space=3)
rect(s, M, 6.14, CW, 0.70, FILL2, BOX)
...
items_ok = [
    ...
    ("組み込みの敵は計算ではない", "踏んだ 6 個の地雷のうち、計算に起因するものはゼロ。全部が電源・接続・RTOS・SDK の仕様"),
...
items_ng = [
    ...
    ("N を大きくした時のスケーリングは未計測", "RAM 330 KB の上限は N≈128。そこまでで通信コストを償却できるかは未検証"),
]
...
res = [(OK, "✓", "標準 MPI API で書いたベンチが、無改変で耳の中で PASS した (OpenMP は互換スタブ)"),
       (OK, "✓", "checksum=32768.000000 厳密一致 / 両バッズ size=2 / EXCEPTION 0 回"),
       (OK, "✓", "5 連タップで再実行、結果は Bluetooth SPP で PC にワイヤレス配信"),
       (NG, "✗", "速くはならなかった (単コア 5〜10 ms → 2 ノード 12〜13 ms) — 理由は定量的に説明できる")]
...
for i, (h, b) in enumerate(nexts):
    tx = M + i * (tw + 0.22)
    panel(s, tx, 4.40, tw, 1.30, [(b, {"space": 0})], size=11.5, head=h, head_size=12.5,
          color=MUTED)
rect(s, M, 5.84, CW, 0.56, FILL2, BOX)
text(s, M, 5.84, CW, 0.56, "みなさんの耳にも、暇をしている Cortex-M4F が 2 個入っています",
     size=17, bold=True, anchor=MSO_ANCHOR.MIDDLE, align=PP_ALIGN.CENTER, pad=0.1)
hline(s, M, 6.54, CW)
text(s, M, 6.62, CW, 0.80, [
