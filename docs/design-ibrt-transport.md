...
### 15.8 実測 (Run 17, 2026-09-01 20:35〜20:42) — omp モードと sniff 外しの評価

ビルド `open_source.bin` 18:02 (`7aa2415` 相当、omp モード + sniff 外し入り)。右→左に書き込み、
両バッズ同時再起動の起動時ラン、ケース内 5 連タップ 2 回 (右 → 左)。生ログは
`~/.claude/handover/artifacts/2026-09-01-run17-omp-sniff/` (`run17-{right,left}.raw`、
`run17-com6.log`)。全ラン (起動時 + タップ計 5 回/側) で **4 モード PASS**、`worker_on_cp` は
比較ラン 1 回ごとに +2 (mpi+omp と omp が各 1 回 CP を fork)。

タップ時 (sysfreq 上昇後、single ≈ 3760 us):

| ラン | single | mpi | mpi+omp | omp | omp 並列領域 (primary/CP待ち) |
|---|---|---|---|---|---|
| タップ #1 右 (rank0) | 3762 us | 110049 us (x0.03) | 130720 us (x0.02) | **2553 us (x1.47)** | 2376 us / 0 us |
| タップ #2 右 (rank0) | 3762 us | **3664 us (x1.02)** | 125733 us (x0.02) | **2553 us (x1.47)** | 2376 us / 0 us |
| タップ #1 左 (rank1) | 3759 us | 130500 us (x0.02) | 129554 us (x0.02) | **2539 us (x1.48)** | 2363 us / 0 us |
| タップ #2 左 (rank1) | 3759 us | 99381 us (x0.03) | 129483 us (x0.02) | **2536 us (x1.48)** | 2364 us / 0 us |

起動時ラン (sysfreq 上昇前、single ≈ 15157 us): omp = 9972〜10025 us で一貫して **x1.51**、
omp 並列領域 primary 9337〜9351 us / CP 待ち 1 us。

**結論**:

- **omp モード (b) は狙いどおり**: リンクを一切使わないので、1 バッズ 2 コアの速度向上率が
  ラン間でぶれずに **x1.47〜1.51** で出る。並列領域 primary が single のほぼ半分
  (2376 us vs 3762 us)、CP 待ち 0〜1 us = CP が MCU より先に自分の半分を終えている。
  §15.6 で mpi+omp の elapsed が TWS 待ちに埋もれて読めなかった「2 コアの効果」が、この 1 行で
  明確に読めるようになった。**これが本フェーズの主要な達成**
- **sniff 外し (a) は mpi+omp を救わなかった**: `app_tws_ibrt_exit_sniff_with_tws()` は機能して
  いる (`was=2 now=0 after 200〜390 ms ok` = リンクモードが SNIFF→ACTIVE に変わったのを確認して
  いる) が、mpi+omp の elapsed は sniff を外したランでも 125〜316 ms のまま。最良の mpi
  (x1.02、3664 us) はむしろ `was=0` (タップ操作でリンクが既に起きていた) のランで出た。
  **原因**: `mpi_ibrt_exit_tws_sniff` を呼ぶのは比較ランの冒頭 1 回だけ。mpi (1 番目の集団通信) は
  その恩恵を受けるが、mpi → MPI_Finalize → mpi+omp と進む間に、集団通信の谷間 (CP fork や
  seams 再設定の数 ms〜十数 ms) でリンクが再び sniff に落ちるため、2 番目の集団通信 mpi+omp は
  また ~300 ms の復帰待ちを食う。§15.7 設計 3 の「4 モード合計は sniff 落ちより短いので無視」の
  前提が、集団通信を 2 つ連続で回すケースでは成り立たなかった
- したがって「MPI 経路の速度向上率」を測りたいなら、(i) sniff 外しを**各集団通信モードの直前**に
  呼ぶ (mpi と mpi+omp の前でそれぞれ exit sniff)、または (ii) そもそも比較ランを 2 コアの
  評価と割り切って omp モードの数字で語る、のどちらか。今回は (ii) を採る (omp モードが
  リンク非依存で安定しており、上位ゴールの Red-Black SOR も omp で 2 コアに割る想定のため)。
  (i) は次フェーズで MPI の実効速度を詰めるときの TODO として残す

**COM6 (SPP)**: 起動時ラン中は #1〜#37 が欠落なく届いた (`run17-com6.log`、GAP 無し、右が master)。
ただし同時再起動でバッズをケースから抜いた瞬間に SPP リンクが切れ、`spp_tail.py` が
`SerialException: ClearCommError failed` (デバイス削除) で落ちたため、**タップ 2 回分は COM6 に
残っていない** (UART キャプチャ `run17-*.raw` には全ラン揃っている)。抜き差しでリブートする
運用上、起動時ランは UART、タップ後の再取得は COM6 を開き直してから、と分けるのが確実。
未達 (Run 16 から継続): タイムアウト経路 `[omp] FAIL worker timeout` は今回も未発生で未確認。
