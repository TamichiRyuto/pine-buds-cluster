...
(`scripts/install-into-sdk.sh` の §12.4 パッチ適用後) でも、**リブートトリガは従来どおり
「出して 3 秒待って戻す」でよい** (2026-09-01 に 2 回実測)。=0 で取り外し時の `app_reset()`
は無効化されるが、充電起動 (`CHARGING PWRON!`) で立ち上がったバッズはケースから外れると
電源ごと落ちるため、再挿入が必ず充電起動のコールドブートになる (design doc §12.7 リスク 3
の訂正記録)。この抜き差し手順は OpenPineBuds README:27 記載の公式手順でもある。
なお README:41 は「ケース内で背面ボタン約 5 秒長押しでも強制再起動できる」と書くが、
**手元の個体にそのようなボタンは存在しない** (2026-09-01 ユーザー確認)。前段落の
「背面ボタン長押しで強制再起動」も同様で、この個体では抜き差しだけを使うこと。
